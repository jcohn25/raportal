...
(function($){ //jquery.ipaddress.js
	$.fn.ipaddress = function(params) {
		return $(this).each(function(){
			...
			/***** added by Kenner Estes, 5/20/2012 *****/
			$.fn.jumpToNext = function() {
				this.next('input.ip_octet').focus().select();
			}
			/***** end added by Kenner Estes *****/
			
			/***** added by Kenner Estes, 5/20/2012 *****/
			$('input.ip_octet').click(function() {
				$(this).select();
			});
			/***** end added by Kenner Estes *****/
			...
			$('input.ip_octet').bind('keydown', function(e){
				...
				/***** added by Kenner Estes, 5/20/2012 *****/
				// jump to next octet if maxlength is reached upon numeric keypress
				if (this.value.length == this.getAttribute('maxlength') - 1 && isNumeric(e)) {
					if (next_octet.length) {
						setTimeout("$('#" + this.id + "').jumpToNext();", 1);
						return true;
					}
				}
				/***** end added by Kenner Estes *****/
				...
				if (e.keyCode == 8 && $(this).caret()[0] == 0 && $(this).caret()[0] == $(this).caret()[1]) {
 				...
 				if (!e.isPropagationStopped()) { // wrapper added by Kenner Estes, 5/23/2012
					if (this.value > 255) this.value = 255;
	
					saveIP(this);
					
					/***** added by Kenner Estes, 5/23/2012 *****/
